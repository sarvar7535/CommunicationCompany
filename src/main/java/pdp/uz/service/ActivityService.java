...
public class ActivityService {
    ...
    public ApiResponse getAll() {
        return new ApiResponse("Ok", true, activityRepo.findAll());
    }


    public ApiResponse get(Long id) {
        Optional<Activity> optionalActivity = activityRepo.findById(id);
        return optionalActivity.map(activity -> new ApiResponse("Ok", true, activity)).orElseGet(() -> new ApiResponse("Activity not found", false));
    }


    public ApiResponse add(ActivityDto dto) {
        try {
            ...
            if (from.getSimCard().getBalance() > 0) {
                ...
                for (int i = 0; i < dto.getAmount(); i++) {
                    if (!simCardService.activity(from, activityType, 1))
                        break;
                    amount++;
                }
    ...
    public ApiResponse getMyActivityExcel(String number, String from, String to, HttpServletResponse response) {
        ...
        try {
            ...
            activities = activityRepo.findAllBySubscriberIdAndCreatedAtBetween(subscriber.getId(), start, end);
